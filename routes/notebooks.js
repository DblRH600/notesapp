import express from 'express'
import NoteBook from '../models/NoteBook.js'
import Note from '../models/Notes.js'
import { authMiddleware } from '../utils/auth.js'

const router = express.Router()

// apply middleware auth to all routes here
router.use(authMiddleware)

// GET /api/notes - get all notes for user
router.get('/', async (req, res) => {
  try {
    const notebooks = await NoteBook.find({ user: req.user._id }).populate(
      'notes'
    )
    res.json(notebooks)
  } catch (err) {
    res.status(500).json(err)
  }
})

// POST /api/notes - note creation route
router.post('/', async (req, res) => {
  try {
    const notebook = await NoteBook.create({
      ...req.body,
      user: req.user._id
    })

    res.status(201).json(notebook)
  } catch (err) {
    res.status(400).json(err)
  }
})

// PUT /api/notebooks/:notebookId/add/:noteId - route for updating notes in notebook
router.put('/:notebookId/add/:noteId', async (req, res) => {
  try {
    const notebook = await NoteBook.findById(req.params.notebookId)

    if (!notebook) {
      return res.status(400).json({ message: 'Notebook cannot be found' })
    }

    // AuthZ check
    if (notebook.user.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({
          message: 'User is not authorized to add notes to this notebook'
        })
    }

    // create a new note
    const note = await Note.findById(req.params.noteId)

    if (!note) {
      return res.status(404).json({ message: 'Note is not found' })
    }

    // add note to notebook
    notebook.notes.push(note._id)
    await notebook.save()

    res.status(201).json(note)
  } catch (err) {
    res.status(500).json(err)
  }
})

export default router;
